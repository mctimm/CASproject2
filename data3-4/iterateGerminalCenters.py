...
def runCode():
    ...
    def closeAntibody(baseVirus, diff):
        anitbody = copy.deepcopy(
            baseVirus[random.randint(0, len(baseVirus)-1)])
    ...
    secondWeight = 1000
    creator.create("FitnessMax", base.Fitness, weights=(1.0, secondWeight))
    ...
    toolbox.register("GerminalCenter", tools.initRepeat,
    ...
    def addHeaderAndTranspose(arr):
        for i, array in enumerate(arr):
            if(i == 0):
                array.insert(0, "time")
            else:
                array.insert(0, "G"+str(i))
        print(arr)
    ...
    def main():
        ...
        newdir = str(numberOfGernimalCenters)+"_germinal_" + \
            "second_weight="+str(secondWeight)
        try:
            os.mkdir(newdir)
        except:
            1 + 1
        ...
        print(totalAveragesVAll)
        f1.close()
        f1 = open("AveragesAgainst1.csv", "w", newline="")
        writer = csv.writer(f1)
        writer.writerows(addHeaderAndTranspose(totalAveragesV1))
        f1.close()
        f1 = open("AverageAgainstAll.csv", "w", newline="")
        writer = csv.writer(f1)
        writer.writerows(addHeaderAndTranspose(totalAveragesVAll))
        f1.close()
